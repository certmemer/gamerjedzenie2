...
void ImGui::Render()
{
	...
	if (g.Style.Alpha > 0.0f)
	{
		...
		if (g.IO.MouseDrawCursor)
		{
			...
			g.OverlayDrawList.AddImage(tex_id, pos, pos + size, cursor_data.TexUvMin[0], cursor_data.TexUvMax[0], IM_COL32(200, 50, 50, 255)); // White fill
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
	...
	ImGuiWindowFlags flags = extra_flags | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		const float window_border_size = 1.f;// window->WindowBorderSize;
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos, float radius)
{
	...
	else //ImVec4 fontColor = ImVec4(1.f, 1.f, 1.f, 1.f);
	{
		window->DrawList->AddLine(center + ImVec2(+cross_extent, +cross_extent), center + ImVec2(-cross_extent, -cross_extent), GetColorU32(ImVec4(0.6f, 0.6f, 0.6f, 1.f)));
		window->DrawList->AddLine(center + ImVec2(+cross_extent, -cross_extent), center + ImVec2(-cross_extent, +cross_extent), GetColorU32(ImVec4(0.6f, 0.6f, 0.6f, 1.f)));
	}
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	const float grab_padding = 1.0f;
	const float slider_sz = is_horizontal ? (frame_bb.GetWidth() - grab_padding * 0.5f) : (frame_bb.GetHeight() - grab_padding * 2.0f);
	float grab_sz;
	if (decimal_precision != 0)
	...
	ImRect grab_bb;
	//	if (is_horizontal)
	grab_bb = ImRect(ImVec2(frame_bb.Min.x + 1, frame_bb.Min.y + grab_padding), ImVec2(grab_pos + 1 + grab_sz * 0.25f, frame_bb.Max.y - grab_padding));
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w + 2, label_size.y + style.FramePadding.y - 1));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	const ImRect text_bb(window->DC.CursorPos + ImVec2(0, style.FramePadding.y - 2), window->DC.CursorPos + ImVec2(0, style.FramePadding.y - 2) + label_size);
	if (label_size.x > 0)
	{
		ItemSize(ImVec2(text_bb.GetWidth(), check_bb.GetHeight()), style.FramePadding.y - 1);
...
bool ImGui::InputScalarEx(const char* label, ImGuiDataType data_type, void* data_ptr, void* step_ptr, void* step_fast_ptr, const char* scalar_format, ImGuiInputTextFlags extra_flags)
{
	...
	if (step_ptr)
		PushItemWidth(ImMax(1.0f, CalcItemWidth() - (button_sz.x + style.ItemInnerSpacing.x)));
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y - 1));//*2.0f));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, id))
		return false;
	...
	//RenderFrame(frame_bb.Min , frame_bb.Max, GetColorU32(popup_open || hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button), true, style.FrameRounding);
	RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
	//RenderFrame(ImVec2(frame_bb.Max.x, frame_bb.Min.y), frame_bb.Max + ImVec2(10,0), GetColorU32(popup_open || hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button), true, style.FrameRounding); // FIXME-ROUNDING
	...
	if (preview_value != NULL)
		RenderTextClipped(frame_bb.Min + style.FramePadding - ImVec2(0, 2), value_bb.Max - ImVec2(0, 1), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
	if (label_size.x > 0)
		RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y - 2), label);
...
{
	const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items);// items_count, height_in_items);
	return value_changed;
}
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
	...
	if (size.x == 0.0f)
		size.x = 18.f; //size.x = default_size;
	if (size.y == 0.0f) size.y = 12.f; //size.y = default_size;
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if ((flags & (ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		const int fmt_idx = 1;// hide_prefix ? 0 : (flags & ImGuiColorEditFlags_HSV) ? 2 : 1;
...
void ImGui::Separator()
{
	...
	window->DrawList->AddLine(ImVec2(bb.Min.x - 5, bb.Min.y), ImVec2(bb.Max.x + 5, bb.Min.y), GetColorU32(ImGuiCol_Separator));
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
	...
	const ImGuiID id = window->GetID(label);
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	ImVec2 size = CalcItemSize(size_arg, CalcItemWidth(), label_size.y + style.FramePadding.y);
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(30, 12));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
	...
	ImVec2 render_pos = frame_bb.Min + style.FramePadding; //ogar rozmiar

	RenderTextClipped(frame_bb.Min + style.FramePadding - ImVec2(9, 2), frame_bb.Max - style.FramePadding - ImVec2(9, 2), buf_display, NULL, &label_size, ImVec2(0.5f, 0.5f), &clip_rect);
	//draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos, GetColorU32(ImGuiCol_Text), buf_display, NULL, 0.0f, &clip_rect);

	if (label_size.x > 0)
		RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - 4, frame_bb.Min.y + style.FramePadding.y), label);
